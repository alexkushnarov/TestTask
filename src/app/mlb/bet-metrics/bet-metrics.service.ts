...
export class BetMetricsService {
  ...
  mlbBetMetrics(bet_type, model, outcome, win_prob_max, win_prob_min, ev_max, ev_min, scale) {
    ...
    const endpoint = `${environment.api_url}/mlb/bet_metrics?bet_id=${bet_type}&model=${model}&outcome=${outcome}`
    + `&win_prob_max=${win_prob_max}&win_prob_min=${win_prob_min}&ev_max=${ev_max}&ev_min=${ev_min}&scale=${scale}`;
